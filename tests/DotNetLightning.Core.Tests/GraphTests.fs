...
let descFromNodes(id, a: NodeId, b: NodeId) =
    {
        ShortChannelId = ShortChannelId.FromUInt64(uint64 id)
        A = a
        B = b
    }
...
let graphTests =
    testList
        "GraphTests from eclair"
        [
            testCase "Instantiate a graph, with vertices and then add edges"
            <| fun _ ->
                let g =
                    DirectedLNGraph
                        .Create()
                ...
                let descBC, updateBC =
                    makeUpdate(2UL, b, c, LNMoney.Zero, 0u, None, None, None)

                let descAD, updateAD =
                    makeUpdate(3UL, a, d, LNMoney.Zero, 0u, None, None, None)

                let descDC, updateDC =
                    makeUpdate(4UL, d, c, LNMoney.Zero, 0u, None, None, None)

                let descCE, updateCE =
                    makeUpdate(5UL, c, e, LNMoney.Zero, 0u, None, None, None)
            ...
            testCase "instantiate a graph adding edges only"
            <| fun _ ->
                let labelAB =
                    makeUpdateSimple(1UL, a, b)
                    |> fun (a, b) ->
            ...
            testCase
                "containsEdge should return true if the graph contains that edge, false otherwise"
            <| fun _ ->
                let updates =
                    seq {
                        makeUpdateSimple(1UL, a, b)
            ...
            testCase "Should remove a set of edges"
            <| fun _ ->
            ...
            testCase "should get an edge given two vertices"
            <| fun _ ->
                let updates =
                    seq {
                        makeUpdateSimple(1UL, a, b)
            ...
            testCase "there can be multiple edges between the same vertices"
            <| fun _ ->
                let graph: DirectedLNGraph = makeTestGraph()
                // A --> B, A --> D
            ...
            testCase
                "remove a vertex with incoming edges and check those edges are removed too"
            <| fun _ ->
                ...
                Expect.isFalse
                    (withoutE.ContainsEdge(descFromNodes(5, c, e)))
                    ""

                Expect.isFalse
                    (withoutE.ContainsEdge(descFromNodes(6, b, e)))
                    ""
        ]
