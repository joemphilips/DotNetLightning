...
let tests =
    let insert
        ...
        let commitmentNumber =
            CommitmentNumber <| UInt48.FromUInt64 commitmentNumber

        let perCommitmentSecret =
            key
            |> hex.DecodeData
            |> fun h -> new Key(h) |> PerCommitmentSecret

        perCommitmentSecretStore.InsertPerCommitmentSecret
            commitmentNumber
            perCommitmentSecret
    ...
    testList
        "Per commitment secret store tests"
        [
            testCase "insert secret correct sequence"
            <| fun _ ->
            ...
            testCase "insert secret 1 incorrect"
            <| fun _ ->
                let res =
                    PerCommitmentSecretStore()
                    |> insertUnwrap
                ...
                match res with
                | Error(SecretMismatch(previous, current)) when
                    previous.Index().UInt64 = 281474976710655UL
                    && current.Index().UInt64 = 281474976710654UL
                    ->
                    ()
                | _ -> failwith <| sprintf "unexpected result: %A" res

            testCase "insert secret 2 incorrect"
            <| fun _ ->
                let res =
                    PerCommitmentSecretStore()
                    |> insertUnwrap
                ...
                | Error(SecretMismatch(previous, current)) when
                    ...
                    && current.Index().UInt64 = 281474976710652UL
                    ->
                    ()
                | _ -> failwith <| sprintf "unexpected result: %A" res

            testCase "insert secret 3 incorrect"
            <| fun _ ->
                let res =
                    PerCommitmentSecretStore()
                    |> insertUnwrap
            ...
            testCase "insert secret 4 incorrect"
            <| fun _ ->
                let res =
                    PerCommitmentSecretStore()
                    |> insertUnwrap
            ...
            <| fun _ ->
                ...
                match res with
                | Error(SecretMismatch(previous, current)) when
                    previous.Index().UInt64 = 281474976710651UL
                    && current.Index().UInt64 = 281474976710650UL
                    ->
                    ()
                | _ -> failwith <| sprintf "unexpected result: %A" res

            testCase "insert secret 6 incorrect"
            <| fun _ ->
                let res =
                    PerCommitmentSecretStore()
                    |> insertUnwrap
                ...
                match res with
                | Error(SecretMismatch(previous, current)) when
                    previous.Index().UInt64 = 281474976710650UL
                    && current.Index().UInt64 = 281474976710648UL
                    ->
                    ()
                | _ -> failwith <| sprintf "unexpected result: %A" res

            testCase "insert secret 7 incorrect"
            <| fun _ ->
                let res =
                    PerCommitmentSecretStore()
                    |> insertUnwrap
                ...
                match res with
                | Error(SecretMismatch(previous, current)) when
                    previous.Index().UInt64 = 281474976710649UL
                    && current.Index().UInt64 = 281474976710648UL
                    ->
                    ()
                | _ -> failwith <| sprintf "unexpected result: %A" res

            testCase "insert secret 8 incorrect"
            <| fun _ ->
                let res =
                    PerCommitmentSecretStore()
                    |> insertUnwrap
