...
let hops2Ids(route: seq<ChannelHop>) =
    route
    |> Seq.map(fun hop ->
        hop.LastUpdateValue.ShortChannelId.ToBytes()
...
let hops2Edges(route: seq<ChannelHop>) =
    route
    |> Seq.map(fun h ->
        {
            GraphLabel.Desc =
...
let tests =
    testList
        ...
        [
            ...
            testCase "Calculate simple route (add and remove edges)"
            <| fun _ ->
                let updates =
                    [
            ...
            testCase "calculate the shortest path (select direct channel)"
            <| fun _ ->
                let updates =
                    [
                        makeUpdate(
            ...
            testCase "route not found (source OR target node not connected)"
            <| fun _ ->
                let updates =
                    [
                        makeUpdateSimple(2UL, b, c)
