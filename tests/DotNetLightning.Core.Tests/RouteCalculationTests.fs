...
let makeChannelAnnCore(shortChannelId: ShortChannelId, nodeIdA, nodeIdB) =
    ...
    { UnsignedChannelAnnouncement.ShortChannelId = shortChannelId
      NodeId1 = nodeId1
      NodeId2 = nodeId2
...
let tests = testList "Route Calculation" [
    ...
    testCase "calculate route and return metadata" <| fun _ ->
        let uab =
            { UnsignedChannelUpdate.ChainHash = Network.RegTest.GenesisHash
              ...
              MessageFlags = 0uy
              ChannelFlags = 0uy
              CLTVExpiryDelta = BlockHeightOffset(1u)
    ...
    testCase "ignore cheaper route when it has more than the requested CLTV limit" <| fun _ ->
        let updates = [
            makeUpdate(1UL, a, b, LNMoney.One, 0u, Some(LNMoney.Zero), None, Some(BlockHeightOffset(50u)))
            makeUpdate(2UL, b, c, LNMoney.One, 0u, Some(LNMoney.Zero), None, Some(BlockHeightOffset(50u)))
            makeUpdate(3UL, c, d, LNMoney.One, 0u, Some(LNMoney.Zero), None, Some(BlockHeightOffset(50u)))
            makeUpdate(4UL, a, e, LNMoney.One, 0u, Some(LNMoney.Zero), None, Some(BlockHeightOffset(9u)))
            makeUpdate(5UL, e, f, LNMoney.MilliSatoshis(5), 0u, Some(LNMoney.Zero), None, Some(BlockHeightOffset(9u)))
            makeUpdate(6UL, f, d, LNMoney.MilliSatoshis(5), 0u, Some(LNMoney.Zero), None, Some(BlockHeightOffset(9u)))
        ]
        let g = DirectedLNGraph.Create().AddEdges(updates)
        let route =
            Routing.findRoute (g) a d DEFAULT_AMOUNT_MSAT 1 (Set.empty) (Set.empty) (Set.empty) {DEFAULT_ROUTE_PARAMS with RouteMaxCLTV = (BlockHeightOffset(28u))} (BlockHeight(400000u))
            |> Result.deref
    ...
    testCase "ignore cheaper route when it grows longer than the requested size" <| fun _ ->
        let updates = [
            makeUpdate(1UL, a, b, LNMoney.One, 0u, Some(LNMoney.Zero), None, (Some (BlockHeightOffset(9u))))
            makeUpdate(2UL, b, c, LNMoney.One, 0u, Some(LNMoney.Zero), None, (Some (BlockHeightOffset(9u))))
            makeUpdate(3UL, c, d, LNMoney.One, 0u, Some(LNMoney.Zero), None, (Some (BlockHeightOffset(9u))))
            makeUpdate(4UL, d, e, LNMoney.One, 0u, Some(LNMoney.Zero), None, (Some (BlockHeightOffset(9u))))
            makeUpdate(5UL, e, f, LNMoney.One, 0u, Some(LNMoney.MilliSatoshis(5)), None, (Some (BlockHeightOffset(9u))))
            makeUpdate(6UL, b, f, LNMoney.One, 0u, Some(LNMoney.MilliSatoshis(5)), None, (Some (BlockHeightOffset(9u))))
        ]
        let g = DirectedLNGraph.Create().AddEdges(updates)
        let route =
    ...
    testCase "Use weight ratios to when computing the edge weight" <| fun _ ->
        ...
        let updates = [
            makeUpdate(1UL, a, b, LNMoney.MilliSatoshis(0), 0u, Some(LNMoney.Zero), None, Some (BlockHeightOffset(13u)))
            makeUpdate(4UL, a, e, LNMoney.MilliSatoshis(0), 0u, Some(LNMoney.Zero), None, Some (BlockHeightOffset(12u)))
            makeUpdate(2UL, b, c, LNMoney.MilliSatoshis(1), 0u, Some(LNMoney.Zero), None, Some (BlockHeightOffset(500u)))
            makeUpdate(3UL, c, d, LNMoney.MilliSatoshis(1), 0u, Some(LNMoney.Zero), None, Some (BlockHeightOffset(500u)))
            makeUpdate(5UL, e, f, LNMoney.MilliSatoshis(2), 0u, Some(LNMoney.Zero), None, Some (BlockHeightOffset(9u)))
            makeUpdate(6UL, f, d, LNMoney.MilliSatoshis(2), 0u, Some(LNMoney.Zero), None, Some (BlockHeightOffset(9u)))
            makeUpdate(7UL, e, c, LNMoney.MilliSatoshis(2), 0u, Some(LNMoney.Zero), Some(largeCap), Some (BlockHeightOffset(12u)))
        ]
        let graph = DirectedLNGraph.Create().AddEdges(updates)
        let r =
    ...
    testCase "Prefer going through an older channel if fees and CLTV are the same" <| fun _ ->
        ...
        let mu(schid, one, two) = 
            makeUpdate2(schid, one, two, LNMoney.MilliSatoshis(1), 0u, (Some LNMoney.Zero), None, (Some (BlockHeightOffset(144u))))
        let updates = [
    ...
    testCase "cost function is monotonic" <| fun _ ->
            ...
            let m =
                ...
                let unsignedChannelUpdate1 =
                    { UnsignedChannelUpdate.ChainHash = uint256.Zero
                      ...
                      MessageFlags = 1uy
                      ChannelFlags = 0uy
                      CLTVExpiryDelta = BlockHeightOffset(14u)
                ...
                let unsignedChannelUpdate2 =
                    { UnsignedChannelUpdate.ChainHash = uint256.Zero
                      ...
                      MessageFlags = 1uy
                      ChannelFlags = 1uy
                      CLTVExpiryDelta = BlockHeightOffset(144u)
        ...
        let routeParams =
            { RouteParams.Randomize = false
              ...
              MaxFeePCT = 0.03
              RouteMaxLength = 6
              RouteMaxCLTV = 1008u |> BlockHeightOffset
