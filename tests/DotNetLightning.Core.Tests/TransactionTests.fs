...
let testList = testList "transaction tests" [
    ...
    testCase "check pre-computed transaction weights" <| fun _ ->
        ...
        ()
    *)
    testCase "check spend handling from local/remote commitment txs" <| fun _ ->
        let rand = Random()
        ...
        let createLocalCommitmentTransaction (commitmentNumber: CommitmentNumber) =
            let perCommitmentPoint =
                let perCommitmentSecret = localChannelPrivKeys.CommitmentSeed.DerivePerCommitmentSecret commitmentNumber
                perCommitmentSecret.PerCommitmentPoint()
            let localCommitmentPubKeys = perCommitmentPoint.DeriveCommitmentPubKeys localChannelPubKeys
        ...
        let revokedLocalCommitmentTx =
            createLocalCommitmentTransaction CommitmentNumber.FirstCommitment

        let remoteCommitmentTx =
            let remotePerCommitmentPoint =
                let remotePerCommitmentSecret = remoteChannelPrivKeys.CommitmentSeed.DerivePerCommitmentSecret commitmentNumber
                remotePerCommitmentSecret.PerCommitmentPoint()
            let remoteRemoteCommitmentPubKeys = remotePerCommitmentPoint.DeriveCommitmentPubKeys localChannelPubKeys
            let remoteLocalCommitmentPubKeys = remotePerCommitmentPoint.DeriveCommitmentPubKeys remoteChannelPubKeys
        ...
        let validateClosingResult (expectedAmount: Money) (errorMsg: string) (result: ClosingHelpers.ClosingResult) =
            let dest =
                use remoteDestPrivKey = new Key()
                remoteDestPrivKey.PubKey.WitHash.ScriptPubKey
            let actualAmount =
                (Result.deref result.MainOutput)
                    .SendAll(dest)
                    .SendFees(Money.Zero)
                    .BuildTransaction(true)
                    .Outputs
                    |> Seq.pick (fun output -> if output.ScriptPubKey = dest then Some output.Value else None)
        ...
        ClosingHelpers.HandleFundingTxSpent
            ...
            remoteChannelPrivKeys
            localCommitmentTx
        |> validateClosingResult (remoteAmount.ToMoney()) "wrong to_remote spending tx"
        ...
        ClosingHelpers.HandleFundingTxSpent
            ...
            remoteChannelPrivKeys
            remoteCommitmentTx
        |> validateClosingResult (remoteAmount.ToMoney()) "wrong to_local spending tx"

        let expectedAmountFromToLocal =
            let localAmount = localCommitmentSpec.ToLocal.ToMoney()
            let fee = revokedLocalCommitmentTx.GetFee [| fundingScriptCoin :> ICoin |]
            localAmount - fee
        let expectedAmountFromToRemote =
        ...
        |> validateClosingResult (expectedAmountFromToRemote + expectedAmountFromToLocal) "wrong penalty tx"
    ...
    testCase "HandleFundingTxSpent should return UnknownClosingTx error for old local commitment txs" <| fun _ ->
        let rand = Random()
        ...
        let createRemoteCommitmentTransaction (commitmentNumber: CommitmentNumber) =
            let perCommitmentSecret = remoteChannelPrivKeys.CommitmentSeed.DerivePerCommitmentSecret commitmentNumber
            let perCommitmentPoint = perCommitmentSecret.PerCommitmentPoint()
            let localCommitmentPubKeys = perCommitmentPoint.DeriveCommitmentPubKeys localChannelPubKeys
            let remoteCommitmentPubKeys = perCommitmentPoint.DeriveCommitmentPubKeys remoteChannelPubKeys
        ...
        let revokedRemoteCommitmentTx =
            createRemoteCommitmentTransaction CommitmentNumber.FirstCommitment
        let remoteCommitmentTx =
        ...
        Expect.isTrue handleFundingTxSpentErrorIsUnknownClosingTx "HandleFundingTxSpent didn't return UnknownClosingTx for an old local commitment tx"
]
