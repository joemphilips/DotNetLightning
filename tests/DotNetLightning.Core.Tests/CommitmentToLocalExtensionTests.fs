...
let commitmentToLocalExtensionTests =
    testList
        "CommitmentToLocalExtensionTests"
        [
            testCase "can extract parameters"
            <| fun _ ->
                let rand = Random()
                ...
                let parametersOpt =
                    CommitmentToLocalParameters.TryExtractParameters
                        scriptPubKey
                ...
                Expect.equal
                    parameters.ToSelfDelay
                    toSelfDelay
                    "to_self_delay mismatch"

                Expect.equal
                    parameters.LocalDelayedPubKey
                    localDelayedPaymentPubKey
                    "local delayed pubkey mismatch"
        ]
