...
let tests =
    ...
    testList
        "PaymentRequest property tests"
        [
            testPropertyWithConfig config "PaymentRequest Serialization"
            <| fun (p: PaymentRequest) ->
                ...
                Expect.equal
                    ...
                    (p2)
                    (sprintf
                        "PaymentRequest before/after serialization is not equal %A"
                        p)
        ]
    ...
    testList
        ...
        [
            testPropertyWithConfig
                config
                "macaroon identifier serialize/deserialize"
            <| fun (i: MacaroonIdentifier) ->
                let i2 =
                    MacaroonIdentifier.TryCreateFromBytes(i.ToBytes())
                    |> Result.deref
