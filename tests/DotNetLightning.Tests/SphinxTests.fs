...
let tests =
    testList "bolt4 tests" [
        ...
        ftestCase "Create packet (reference test vector)" <| fun _ ->
            let (onion, ss) =
                let p = PacketAndSecrets.Create log (sessionKey, pubKeys, payloads, associatedData)
            ...
            let expectedPacket =
                ...
                |> hex.DecodeData
            Expect.equal (onion.ToBytes()) (expectedPacket) ""
            let { Payload = payload0; NextPacket = nextPacket0; SharedSecret = ss0 }: ParsedPacket =
                Sphinx.parsePacket (privKeys.[0]) (associatedData) (onion.ToBytes()) |> RResult.rderef
