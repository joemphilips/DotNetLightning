...
def gen_primitive(p):
    ...
    if p.required:
        if converter_name is not None:
            defi += f'    [<System.Text.Json.Serialization.JsonConverter(typeof<SystemTextJsonConverters.{converter_name}>)>]\n'
            defi += f'    [<Newtonsoft.Json.JsonConverter(typeof<NewtonsoftJsonConverters.{converter_name}>)>]\n'
        defi += f'    [<JsonPropertyName("{org}")>]\n    {to_PascalCase(p.name.normalized())}: {typename}\n'
    else:
        defi += '    [<Newtonsoft.Json.JsonConverter(typeof<NewtonsoftJsonConverters.OptionConverter>)>]\n'
        defi += '    [<JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)>]\n'
