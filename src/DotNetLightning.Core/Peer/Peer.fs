...
type Peer =
    {
        ...
        TheirFeatures: option<FeatureBits>
    ...
    static member CreateOutbound(theirNodeId, ourNodeSecret: Key) =
        {
            ChannelEncryptor =
                PeerChannelEncryptor.newOutBound(theirNodeId, ourNodeSecret)
            IsOutBound = true
...
module Peer =
    let executeCommand (state: Peer) (cmd: PeerCommand) =
        ...
        | NoiseComplete, DecodeCipherPacket(lengthPacket, reader) ->
            result {
                let! (len, pce) =
                    PeerChannelEncryptor.decryptLengthHeader
                        lengthPacket
                        state.ChannelEncryptor
                ...
                let! (b, newPCE) =
                    pce |> PeerChannelEncryptor.decryptMessage packet

                let! msg =
                    LightningMsg.fromBytes b
                    |> Result.mapError(PeerError.P2PMessageDecodeError)

                return
                    match msg with
                    | :? ErrorMsg as msg -> [ ReceivedError(msg, newPCE) ]
                    | :? PingMsg as msg -> [ ReceivedPing(msg, newPCE) ]
                    | :? PongMsg as msg -> [ ReceivedPong(msg, newPCE) ]
                    | :? InitMsg as msg -> [ ReceivedInit(msg, newPCE) ]
                    | :? IRoutingMsg as msg ->
                        [ ReceivedRoutingMsg(msg, newPCE) ]
                    | :? IChannelMsg as msg ->
                        [ ReceivedChannelMsg(msg, newPCE) ]
                    | _ -> failwithf "unreachable %A" msg
            }
        | NoiseComplete, EncodeMsg msg ->
            state.ChannelEncryptor
            |> PeerChannelEncryptor.encryptMessage(msg.ToBytes())
            |> (MsgEncoded >> List.singleton >> Ok)
        | _, cmd ->
            failwithf
                "Peer does not know how to handle %A while in noise step %A"
                cmd
                noiseStep
