...
module Amount =
    ...
    let decode(input: string) : Result<LNMoney, string> =
        let parseCore (a: string) multi =
            ...
            | true, aValue -> LNMoney.MilliSatoshis(aValue * multi) |> Ok
        ...
        | a when a.[a.Length - 1] = 'p' ->
            ...
            |> Result.map(fun lnMoney ->
                (lnMoney.MilliSatoshi / 10L) |> LNMoney.MilliSatoshis
            )
        ...
        | a -> parseCore a 100000000000L

    let encode(amount: option<LNMoney>) =
        match amount with
        | None -> ""
        | Some amt when unit(amt) = 'p' ->
            sprintf "%ip" (amt.MilliSatoshi * 10L)
        | Some amt when unit(amt) = 'n' ->
            sprintf "%in" (amt.MilliSatoshi / 100L)
        | Some amt when unit(amt) = 'u' ->
            sprintf "%iu" (amt.MilliSatoshi / 100000L)
        | Some amt when unit(amt) = 'm' ->
            sprintf "%im" (amt.MilliSatoshi / 100000000L)
