...
type ServiceSatisfier(targetService: string) =
    do checkNull "targetService" targetService
    ...
    interface ISatisfier with
        ...
        member this.SatisfyPrevious(prev, curr) =
            result {
                ...
                let! prevServices =
                    Service.ParseMany(prevServiceValue.ToString())
                ...
                let! currentServices =
                    Service.ParseMany(currentServiceValue.ToString())
                ...
                for s in currentServices do
                    if not <| prevServices.Contains(s) then
                        return!
                            Error(
                                sprintf
                                    "Service (%s) was not previously allowed!"
                                    s.Name
                            )
                    else
                        ()
        ...
        member this.SatisfyFinal finalCaveat =
            result {
                ...
                if services |> Seq.exists(fun s -> s.Name = targetService) then
                    ...
                    return!
                        Error(
                            sprintf "Target service %s not found" targetService
                        )
            }
...
type CapabilitiesSatisfier(service: string, targetCapability: string) =
    ...
    member val Condition =
        sprintf "%s%s" service Constants.CAPABILITIES_CONDITION_PREFIX
                ...
                for c in currentCapabilities do
                    if (not <| prevCapabilities.Contains(c)) then
                        return!
                            Error(
                                sprintf
                                    "Capability (%A) was not previously allowed!"
                                    c
                            )
...
type MacaroonExtensions() =
    ...
    [<Extension>]
    static member VerifyLSATCaveats
        (
            macaroon: Macaroon,
