...
type Service =
        ...
        if (String.IsNullOrEmpty(s)) then
            ...
            result {
                ...
                for rawService in rawServices do
                    ...
                    if serviceInfo.Length <> 2 then
                        return!
                            Error(
                                sprintf
                                    "Invalid value for service %A"
                                    serviceInfo
                            )
                    else
                        ...
                        let! t =
                            ...
                            | false, _ ->
                                Error(
                                    sprintf
                                        "invalid service caveat value %s Service tier must be uint8 (%s)"
                                        s
                                        tier
                                )
    ...
    static member EncodeToCaveat(services: IList<Service>) : Result<Caveat, _> =
        result {
            if (services.Count = 0) then
            ...
            else if (String.IsNullOrEmpty services.[0].Name) then
                ...
                sb.Append(
                    sprintf "%s:%i" services.[0].Name services.[0].ServiceTier
                )
                |> ignore

                for s in services do
                    if (String.IsNullOrEmpty s.Name) then
                        return! Error("Missing service name")
                    else
                        sb.Append(sprintf ",%s:%i" s.Name s.ServiceTier)
                        |> ignore
