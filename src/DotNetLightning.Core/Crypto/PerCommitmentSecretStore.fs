...
type InsertPerCommitmentSecretError =
    | UnexpectedCommitmentNumber of
        got: CommitmentNumber *
        expected: CommitmentNumber
    | SecretMismatch of
        previousCommitmentNumber: CommitmentNumber *
        newCommitmentNumber: CommitmentNumber
...
type PerCommitmentSecretStore
    private
    (
        secrets: list<CommitmentNumber * PerCommitmentSecret>
    ) =
    new() = PerCommitmentSecretStore(List.empty)

    member this.Secrets = secrets

    static member FromSecrets
        (secrets: list<CommitmentNumber * PerCommitmentSecret>)
        ...
        if not(sanityCheck commitmentNumbers) then
            failwithf
                "commitment number list is malformed: %A"
                commitmentNumbers
    ...
    member this.InsertPerCommitmentSecret
        ...
        if commitmentNumber <> nextCommitmentNumber then
            ...
            let rec fold
                ...
                if secrets.IsEmpty then
                    let res =
                        [
                            commitmentNumber, perCommitmentSecret
                        ]

                    Ok <| PerCommitmentSecretStore res
                else
                    let storedCommitmentNumber, storedPerCommitmentSecret =
                        secrets.Head

                    match
                        perCommitmentSecret.DeriveChild
                            commitmentNumber
                            storedCommitmentNumber
                        with
                    | Some derivedPerCommitmentSecret ->
                        if derivedPerCommitmentSecret
                           <> storedPerCommitmentSecret then
                            Error
                            <| SecretMismatch(
                                storedCommitmentNumber,
                                commitmentNumber
                    ...
                    | None ->
                        let res =
                            (commitmentNumber, perCommitmentSecret) :: secrets

                        Ok <| PerCommitmentSecretStore res

            fold this.Secrets

    member this.GetPerCommitmentSecret
        (commitmentNumber: CommitmentNumber)
        ...
        let rec fold(secrets: list<CommitmentNumber * PerCommitmentSecret>) =
            if secrets.IsEmpty then
                ...
                match
                    storedPerCommitmentSecret.DeriveChild
                        storedCommitmentNumber
                        commitmentNumber
                    with
