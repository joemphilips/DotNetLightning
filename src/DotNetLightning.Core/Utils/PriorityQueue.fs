...
type IPriorityQueue<'T when 'T: comparison> =
    ...
    abstract member TryPeek: unit -> option<'T>
    abstract member Peek: unit -> 'T
    abstract member Length: int
    abstract member TryPop: unit -> option<('T * IPriorityQueue<'T>)>
...
type Heap<'T when 'T: comparison>
    ...
    static member private foldHeap nodeF leafV (h: list<HeapData<'T>>) =
        let rec loop (h: list<HeapData<'T>>) cont =
            ...
            | T(a, h') :: tl ->
                loop
                    h'
                    (fun lacc -> loop tl (fun racc -> cont(nodeF a lacc racc)))
            | _ -> cont leafV

        loop h (id)


    static member private inOrder(h: list<HeapData<'T>>) =
        (Heap.foldHeap (fun x l r acc -> l(x :: (r acc))) id h) []

    static member ofSeq (isDescending: bool) (s: seq<'T>) : Heap<'T> =
        if Seq.isEmpty s then
            Heap(isDescending, 0, E)
        else
            let len, h' =
                Seq.fold
                    (fun (lnth, (h: HeapData<'T>)) x ->
                        match h with
                        | E -> 1, T(x, [])
                        | T(y, ys) ->
                            if isDescending then
    ...
    member this.Merge(xs: Heap<'T>) =
        if isDescending = xs.IsDescending then
            Heap.merge isDescending (length + xs.HeapLength) data xs.HeapData
        else
            failwith "heaps to merge have different sort orders"
    ...
    member this.Rev() =
        if isDescending then
            Heap<'T>.ofSeq false (this :> seq<'T>)
        else
            Heap<'T>.ofSeq true (this :> seq<'T>)
        ...
        member this.GetEnumerator() =
            let e =
                ...
                if isDescending then
                    ...
                    Heap.inOrder listH |> List.sort |> List.toSeq

            e.GetEnumerator()

        member this.GetEnumerator() =
            (this :> seq<_>).GetEnumerator() :> IEnumerator
    ...
    interface IPriorityQueue<'T> with
        ...
        member this.TryPop() : option<('T * IPriorityQueue<'T>)> =
...
module PriorityQueue =
    ...
    let empty<'T when 'T: comparison> isDescending =
        Heap.empty isDescending :> IPriorityQueue<'T>

    ///O(1). Returns true if the queue has no elements.
    let inline isEmpty(pq: IPriorityQueue<'T>) =
        pq.IsEmpty

    ///O(log n) amortized time. Returns a new queue with the element added to the end.
    let inline insert element (pq: IPriorityQueue<'T>) =
        pq.Insert element

    ///O(1). Returns option first element.
    let inline tryPeek(pq: IPriorityQueue<'T>) =
        pq.TryPeek()

    ///O(1). Returns the first element.
    let inline peek(pq: IPriorityQueue<'T>) =
        pq.Peek()

    ///O(log n) amortized time. Returns the option first element and tail.
    let inline tryPop(pq: IPriorityQueue<'T>) =
        pq.TryPop()

    ///O(log n) amortized time. Returns the first element and tail.
    let inline pop(pq: IPriorityQueue<'T>) =
        pq.Pop()
