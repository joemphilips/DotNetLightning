...
type ConsoleExecutionLog() =
    ...
    override __.OnPreparingErrand(errand) = write ("    Preparing errand " + string errand)
    override __.OnPreparedErrand(errand) = write ("    Preparing errand " + string errand)
...
type ExecutionInstance (log: ExecutionLog) =
    ...
    default __.CreateCache() = DefaultCache() :> Cache
...
type private Step(instance: ExecutionInstance, context: ServiceContext, cache: Cache) =
    ...
    member __.Execute token =
        ...
        for group in grouped.Values do
            ...
            i <- i + 1
        for ungroup in ungrouped do
            all.[i] <- upcast ungroup token
            i <- i + 1
        Task.WhenAll(all)
