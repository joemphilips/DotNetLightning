...
type IReplaySerializer =
    ...
    abstract member Deserialize : blog : byte array -> 'a
...
type RecordingExecutionStrategy
    ...
    static member Create
        ...
        RecordingExecutionStrategy.Create(execution, serializer, fun s arr -> saveBlob.Invoke(s, fun () -> arr()))
...
let replay (config: ExecutionConfig) (serializer: IReplaySerializer) (blob: byte array) =
    ...
    let config = { config with Instance = fun () -> ReplayingInstance(config.Instance(), history.Entries) :> ExecutionInstance }
    execute config plan