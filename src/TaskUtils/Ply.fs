...
module Internal =
    ...
    type Awaitable<'u>() =
        abstract member Await<'t when 't :> IAwaitingMachine> :
            machine: byref<'t> -> bool
...
type Ply<'u> =
    ...
    new(await: Internal.Awaitable<'u>) =
        {
    ...
    member this.IsCompletedSuccessfully =
        Object.ReferenceEquals(this.awaitable, null)

    member this.Result =
        if this.IsCompletedSuccessfully then
            this.value
        else
            this.awaitable.GetNext().Result
...
module TplPrimitives =
    ...
    [<Struct>]
    type ContinuationStateMachine<'u> =
#if NETSTANDARD2_0
        ...
        interface IAsyncStateMachine with
            ...
            member this.MoveNext() =
                ...
                try
                    if isNotNull this.continuation then
                        this.UnsafeExecuteToFirstYield()
    ...
    and [<Sealed>] TplAwaitable<'methods, 'awt, 't, 'u when 'methods :> IAwaiterMethods<'awt, 't> and 'awt :> ICriticalNotifyCompletion> =
        ...
        override this.GetNext() =
            Debug.Assert(
                this.awaiterMethods.IsCompleted &this.awaiter
                || (typeof<'awt> = typeof<YieldAwaitable.YieldAwaiter>),
                "Forcing an async here"
            )

            this.continuation(this.awaiterMethods.GetResult &this.awaiter)

    and [<Sealed>] PlyAwaitable<'t, 'u>
        (
            awaitable: Awaitable<'t>,
            continuation: 't -> Ply<'u>
        ) =
        inherit Awaitable<'u>()
        let mutable awaitable = awaitable

        override __.Await csm =
            awaitable.Await(&csm)
    ...
    and [<Sealed>] LoopAwaitable
        (
            initialAwaitable: Awaitable<unit>,
            cond: unit -> bool,
            body: unit -> Ply<unit>
        ) =
    ...
    let forLoop (sequence: seq<'a>) (body: 'a -> Ply<unit>) =
        using
    ...
    and [<Struct>] GenericAwaiterMethods<'awt, 't when 'awt :> ICriticalNotifyCompletion> =
        interface IAwaiterMethods<'awt, 't> with
            member __.IsCompleted awt =
                false // Always await, this way we don't have to specialize per awaiter

            member __.GetResult awt =
                defaultof<_> // Always unit because we wrap this continuation to always be unit -> Ply<'u>
    ...
    type Binder<'u>() =
        ...
        static member inline Specialized<'methods, ^awt, 't when 'methods :> IAwaiterMethods< ^awt, 't > and ^awt :> ICriticalNotifyCompletion and ^awt: (member get_IsCompleted:
        ...
        static member inline Generic(task: ^taskLike, cont: 't -> Ply<'u>) =
            ...
            if (^awt: (member get_IsCompleted: unit -> bool) (awt)) then
                ...
                Binder<'u>
                    .Await<GenericAwaiterMethods<_, _>, _, _>(
                        &mutAwt,
                        (fun () ->
                            cont(^awt: (member GetResult: unit -> 't) (awt))
                        )
    ...
    type Bind() =
        ...
        static member inline Invoke(task, cont: 't -> Ply<'u>) =
            let inline call_2(task: ^b, cont, a: ^a) =
                ((^a or ^b): (static member Bind: _ * _ * _ -> Ply<'u>) task,
                                                                        cont,
                                                                        a)

            let inline call(task: 'b, cont, a: 'a) =
                call_2(task, cont, a)
        ...
        static member inline Bind
            ...
            Binder<'u>
                .Specialized<UnitTaskAwaiterMethods<_>, _, _>(
                    task.GetAwaiter(),
                    cont
                )
        ...
        static member inline Bind
            ...
            Binder<'u>
                .Specialized<TaskAwaiterMethods<_>, _, _>(
                    task.GetAwaiter(),
                    cont
                )
        ...
        static member inline Bind
            (
                task: ConfiguredTaskAwaitable<'t>,
                cont: 't -> Ply<'u>,
                [<Optional>] _impl: Bind
            ) =
            Binder<'u>
                .Specialized<ConfiguredTaskAwaiterMethods<_>, _, _>(
                    task.GetAwaiter(),
                    cont
                )
        ...
        static member inline Bind
            (
                task: ConfiguredTaskAwaitable,
                cont: unit -> Ply<'u>,
                [<Optional>] _impl: Bind
            ) =
            Binder<'u>
                .Specialized<ConfiguredUnitTaskAwaiterMethods<_>, _, _>(
                    task.GetAwaiter(),
                    cont
                )
        ...
        static member inline Bind
            ...
            Binder<'u>
                .Specialized<YieldAwaiterMethods<_>, _, _>(
                    task.GetAwaiter(),
                    cont
                )
        ...
        static member inline Bind
            ...
            Binder<'u>
                .Specialized<ValueTaskAwaiterMethods<_>, _, _>(
                    task.GetAwaiter(),
                    cont
                )
        ...
        static member inline Bind
            ...
            Binder<'u>
                .Specialized<UnitValueTaskAwaiterMethods<_>, _, _>(
                    task.GetAwaiter(),
                    cont
                )
        ...
        static member inline Bind
            (
                task: ConfiguredValueTaskAwaitable<'t>,
                cont: 't -> Ply<'u>,
                [<Optional>] _impl: Bind
            ) =
            Binder<'u>
                .Specialized<ConfiguredValueTaskAwaiterMethods<_>, _, _>(
                    task.GetAwaiter(),
                    cont
                )
        ...
        static member inline Bind
            (
                task: ConfiguredValueTaskAwaitable,
                cont: unit -> Ply<'u>,
                [<Optional>] _impl: Bind
            ) =
            Binder<'u>
                .Specialized<ConfiguredUnitValueTaskAwaiterMethods<_>, _, _>(
                    task.GetAwaiter(),
                    cont
                )
#endif
    ...
    type AwaitableBuilder() =
        member inline __.Delay(body: unit -> Ply<'t>) =
            body
        ...
        member inline __.TryFinally
            (
                body: unit -> Ply<'t>,
                finallyBody: unit -> unit
            ) =
