    ...
    member private this.HandleSetupMsgAsync(peerId, msg: ISetupMsg, pipe: IDuplexPipe) =
        vtask {
            ...
            | :? Init as init ->
                if (init.GlobalFeatures.RequiresUnknownBits()) then
                    ...
                    let theirNodeId = if peer.TheirNodeId.IsSome then peer.TheirNodeId.Value else
                                        ...
                                        log.LogError msg
                                        failwith msg
                    this.EventAggregator.Publish<PeerEventWithContext>({ PeerEvent = ReceivedInit (init); NodeId = theirNodeId })
                    ...
                    if (not peer.IsOutBound) then
                        ...
                        this.EventAggregator.Publish<PeerEventWithContext>({ PeerEvent = Connected; NodeId = theirNodeId })
                        return Good ()
            | :? ErrorMessage as e ->
                ...
                if (e.ChannelId = WhichChannel.All) then
                    ...
                    this.EventAggregator.Publish<PeerEventWithContext>({ PeerEvent = ReceivedError(e); NodeId = peer.TheirNodeId.Value } )
                    return Good ()
            | :? Ping as ping ->
                this.EventAggregator.Publish<PeerEventWithContext>({ PeerEvent = ReceivedPing (ping); NodeId = peer.TheirNodeId.Value })
            ...
            | :? Pong as pong ->
                ...
                this.EventAggregator.Publish<PeerEventWithContext>({ PeerEvent = ReceivedPong (pong); NodeId = peer.TheirNodeId.Value })
                return Good ()
                    ...
                    | Good (data) ->
                        ...
                        | Good msg ->
                            ...
                            | :? IRoutingMsg as routingMsg ->
                                this.EventAggregator.Publish<PeerEventWithContext>({ PeerEvent = ReceivedRoutingMsg routingMsg; NodeId = peer.TheirNodeId.Value })
                                return Good ()
                            | :? IChannelMsg as channelMsg ->
                                this.EventAggregator.Publish<PeerEventWithContext>({ PeerEvent = ReceivedChannelMsg (channelMsg); NodeId = peer.TheirNodeId.Value })
                                return Good ()
